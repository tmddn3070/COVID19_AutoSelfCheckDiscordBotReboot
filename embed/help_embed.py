...
help_embed.add_field(name="!정보등록", value="!정보등록 [이름] [생년월일] [지역] [학교이름] [학교타입] [비밀번호]```!정보등록 홍길동 721027 서울시 길동고 고등학교 1234``````!정보등록 홍길동 050201 충청남도 길동중 중학교 2580```※정보등록은 `개인 DM`에서만 작동합니다.", inline=False)
help_embed.add_field(name="!정보삭제", value="※디스코드 아이디를 기준으로 삭제합니다. 디스코드 계정이 바뀌었을 경우엔 관리자에게 문의하세요. \n", inline=False)
help_embed.add_field(name="!정보확인", value="※디스코드 아이디를 기준으로 확인합니다.디스코드 계정이 바뀌었을 경우엔 관리자에게 문의하세요. \n", inline=False)
